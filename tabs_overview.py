...
- All charts use width="stretch" (Streamlit handles responsive width)
- Height is capped via DEFAULT_FIG_HEIGHT for legibility on phones
- Long tables are inside expanders so the main scroll isn’t crazy
"""
...
def render_overview(
    ...
    Layout philosophy:
    - Short intro at top
    - ETFs first (core for many investors)
    - Metals, then FX snapshot, then Crypto
    - Charts full-width, moderate height for mobile friendliness
    """
    ...
    # ETFs – TOP SECTION
    # ========================================================
    st.markdown("### 📊 ETFs – VOO, QQQ, SCHD, IWM, TLT, VXUS (Stooq, EOD)")
    ...
    if etf_error:
        ...
        if not norm_rows:
            ...
            # Metrics row: will stack nicely on mobile
    ...
    # ========================================================
    st.markdown("### ₿ Crypto – BTC & ETH (Yahoo Finance via yfinance)")
